...
using RWS.BackendExtension.ClonePublication.Models;
using System;
...
using Trisoft.InfoShare.API.Interfaces.API25;
...
{
    public class ClonePublicationController : ApiControllerBase
    {
        ILogger<ClonePublicationController> _logger;

        private readonly IApplication25 _application25;
        private IEventMonitor _eventMonitor;
        private long _progressId;


        private string pubName = string.Empty;
        private string pubRootMap = string.Empty;
        ...
        private string pubType = string.Empty;
        private string pubResources = string.Empty;
        private string pubConditions = string.Empty;


        private List<Models.IshObjectDetails> SourceObjects = new List<Models.IshObjectDetails>();
        private List<Models.ObjectSpecificMetadata> objSpecificMetadata = new List<Models.ObjectSpecificMetadata>();
        ...
        public ClonePublicationController(ILogger<ClonePublicationController> logger, IHttpContextAccessor httpContextAccessor) : base(logger, httpContextAccessor)
        {
            _logger = logger;
            Logger.LogDebug("ClonePublication Controller");
        }

        [HttpPost("CopyPublication", Name = "CopyPublication")]
        ...
        public IActionResult CopyPublication(Publication publication)
        {
            ...
            try
            {
                if (publication._publicationId != string.Empty || publication._publicationId != null)
                {
                    ...
                    pubIds.Add(publication._publicationId);
                    ...
                    if (_publicationDetails.SelectSingleNode("//ishobject") == null)
                    {
                        //  _eventMonitor.AddEventDetail(_progressId, EventLevel.Information, "No Publication outputs found for the publication. Please create atleast one output for successful cloning", sourcePubDetails, DetailStatus.Success);
                    ...
                    else
                    {
                        ...
                        pubRootMap = _publicationDetails.SelectNodes("//ishfield[@name='FISHMASTERREF']")[0].InnerText;
                        pubType = _publicationDetails.SelectNodes("//ishfield[@name='FISHPUBLICATIONTYPE']")[0].InnerText;
                        if (_publicationDetails.SelectNodes("//ishfield[@name='FISHRESOURCES']") != null && _publicationDetails.SelectNodes("//ishfield[@name='FISHRESOURCES']")[0] != null)
                            pubResources = _publicationDetails.SelectNodes("//ishfield[@name='FISHRESOURCES']")[0].InnerText;
                        if (_publicationDetails.SelectNodes("//ishfield[@name='FISHPUBCONTEXT']") != null && _publicationDetails.SelectNodes("//ishfield[@name='FISHPUBCONTEXT']")[0] != null)
                            pubConditions = _publicationDetails.SelectNodes("//ishfield[@name='FISHPUBCONTEXT']")[0].InnerText;
                        ...
                        foreach (Models.IshObjectDetails _objRef in SourceObjects)
                        {
                            ...
                            if (_objType != "ISHLibrary" && _objType!= "ISHIllustration")
                            {
                                string _folderRef = CheckAndCreateFolder(publication._newFolderId, _objType);
                                ...
                                Models.ObjectSpecificMetadata reqMetaObject = objSpecificMetadata.Find(x => x.MetaObjType == _objType);
                                string _metadataList = string.Empty;
                                if (reqMetaObject != null)
                                {
                                    _metadataList = reqMetaObject.MetaObjMetadataList;
                                }
                                else
                                {
                                    _metadataList = GetRequiredMetadataforIshType(_objType);
                                }
                                ...
                                string _cuurentObjMetadata = docObj.GetMetadata(_ObjIshRef, ref _objVersionRef, pubWorkingLang, "Low", GenerateMetadataReqXML(_listOfMadatoryMedata, _objType));
                                ...
                                docObj.Create(long.Parse(_folderRef), GetObjectType(_objType), ref _newObjLogId, ref _defaultobjVersion, pubWorkingLang, _objRef.ObjResolution, _createReqMetadata, _objDataEDT, _updatedDataByte);
                        ...
                        Models.IshObjectDetails _MapObject = SourceObjects.Find(x => x.ObjSourceLogId == pubRootMap);
...
/* string newPubMeta = "<ishfields>" +
                        "<ishfield name='FTITLE' level='logical'>" + pubName + "</ishfield>" +
                                                ...
                                                "<ishfield name='FISHPUBLICATIONTYPE' level='logical'>" + pubType + "</ishfield>" +
                                                "<ishfield name='FISHRESOURCES' level='version'>" + pubResources + "</ishfield>" +
                                                "<ishfield name='FPSINREVIEW' level='version'>" + "No" + "</ishfield>" +
                                                "<ishfield name='FISHPUBCONTEXT' level='version'>" + encodeString(pubConditions) + "</ishfield>" +
                                            "</ishfields>";
*/
                        ...
                        string _folderlocation = CheckAndCreateFolder(publication._newFolderId, "ISHPublication");
                ...
                {
                    _logger.LogInformation("No language card ids found in the supplied publication id  '{0}'.", publication._publicationId.ToString());
                }
                // Add a message to the logging pipeline
                _logger.LogInformation("Clone publication has finished successfully");


                var response = new
                {
                    NewPublicationGUID = newPublicationGUID
                };
                return Ok(response);
            ...
            {
                // Add a message to the logging pipeline
                _logger.LogInformation($"Clone publication has failed :'{exception}'");


                var errorResponse = new
                {
                    StatusCode = 500,
                    Message = ((Trisoft.InfoShare.API25.InfoShareException)exception).BaseDescription.ToString()
                };

                _logger.LogInformation($"Clone publication Error '{exception.Message}'");
        ...
        private string GenerateMetadataReqXML(XmlNodeList _sourceMedataList, string _objType)
        {
            ...
            if (_objType == "ISHIllustration")
            {
                metadataXmlReq += "<ishfield name='FILLUSTRATIONTYPE' level='logical' />";
            }
            else if (_objType == "ISHModule")
            {
                metadataXmlReq += "<ishfield name='FMODULETYPE' level='logical' />";
            }
            else if (_objType == "ISHMasterDoc")
            {
                metadataXmlReq += "<ishfield name='FMASTERTYPE' level='logical' />";
            }
        ...
        private string GetRequiredMetadataforIshType(string requiredType)
        {
            ...
            Models.ObjectSpecificMetadata objMeta = new Models.ObjectSpecificMetadata
            {
                MetaObjType = requiredType,
                MetaObjMetadataList = _allMetadata
            };

            objSpecificMetadata.Add(objMeta);
        ...
        private bool IsObjectExist(string logicalId, string objVersion, Trisoft.InfoShare.API25.DocumentObj documentObj25)
        {
            string xmlRequestedMetadata = "<ishfields>" +
                     "<ishfield name='FRESOLUTION' level='lng'/>" +
                    "</ishfields>";
            string xmlObjectList = string.Empty;
            return documentObj25.TryGetMetadata(logicalId, ref objVersion, pubWorkingLang, "Low", xmlRequestedMetadata, out xmlObjectList);
            // XDocument xObjectContent = XDocument.Parse(xmlObjectList);
        ...
        private string GetObjectDetails(string logicalId, string objVersion, Trisoft.InfoShare.API25.DocumentObj documentObj25)
        {
            ...
            try
            {
                //Commented the below code as we had to pass low forceably 
                //     _resObjList = documentObj25.GetObject(logicalId, ref objVersion, pubWorkingLang, "", null, "<ishfields><ishfield name='FRESOLUTION' level='lng'/></ishfields>");
            
                _resObjList = documentObj25.GetObject(logicalId, ref objVersion, pubWorkingLang, "Low", "", "<ishfields><ishfield name='FRESOLUTION' level='lng'/></ishfields>");
            }
            catch (Exception ex)
            {
                //  throw ex;
        ...
        private List<Models.IshObjectDetails> GetAllBaslineObjects(XmlDocument objXML, Trisoft.InfoShare.API25.DocumentObj documentObj25)
        {
            ...
            foreach (XmlNode baseObj in _baseObjNodes)
            {
                ...
                // IsObjectExist(_objRef, _objVar, documentObj25);
                if (IsObjectExist(_objRef, _objVar, documentObj25))
                {
                    ...
                    string generateGUID = "GUID-" + System.Guid.NewGuid().ToString().ToUpper();

                    XmlNodeList _objTypeIshObjectList = _sourceObjXML.SelectNodes("//ishobject");

                    foreach (XmlNode _objTypeIshObject in _objTypeIshObjectList)
                    {
                        string _ObjIshRef = _objTypeIshObject.Attributes["ishref"].Value;
                        string _objType = _objTypeIshObject.Attributes["ishtype"].Value;

                        XmlNode _tempNode = _objTypeIshObject.Clone();

                        string _objResolution = _tempNode.SelectNodes("//ishfield[@name='FRESOLUTION']")[0] != null ? _tempNode.SelectNodes("//ishfield[@name='FRESOLUTION']")[0].InnerText : string.Empty;
                        string _objData = _tempNode.SelectNodes("//ishdata")[0].InnerText;
                        string _objDataEDT = _tempNode.SelectNodes("//ishdata")[0].Attributes["edt"].Value;
                        ...
                        if (!objectGUIDMaps.ContainsValue(newGUID))
                        {
